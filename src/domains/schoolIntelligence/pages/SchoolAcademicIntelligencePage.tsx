...
type Period="ALL"|"7"|"14"|"21"|"30"|"60"|"CUSTOM";
type SortMode="DEFAULT"|"HIGHEST_DOUBTS";
const palettes=[
...
export default function SchoolAcademicIntelligencePage(){
 ...
 const [classroom,setClassroom]=useState("ALL"),[subject,setSubject]=useState("ALL");
 const [period,setPeriod]=useState<Period>("ALL"),[sort,setSort]=useState<SortMode>("DEFAULT");
 const [from,setFrom]=useState(""),[to,setTo]=useState("");
 useEffect(()=>{void load()},[period,from,to]);
 async function load(){if(period==="CUSTOM"&&(!from||!to))return;setLoading(true);try{
   const days=period==="ALL"||period==="CUSTOM"?undefined:Number(period) as 7|14|21|30|60;
   const d=period==="CUSTOM"?await loadSchoolIntelligence(undefined,from,to):await loadSchoolIntelligence(days);
   setAll(d.examPreparation??[]);
 }finally{setLoading(false)}}
 const classOptions=useMemo(()=>all.map(x=>({key:x.classroomKey,label:x.classroom})),[all]);
 const subjects=useMemo(()=>Array.from(new Set(all.flatMap(x=>x.subjects.map(s=>s.subjectName)))).sort(),[all]);
 const visible=useMemo(()=>all.filter(x=>classroom==="ALL"||x.classroomKey===classroom).map(x=>({...x,subjects:x.subjects.filter(s=>subject==="ALL"||s.subjectName===subject).map(s=>({...s,students:sort==="HIGHEST_DOUBTS"?[...s.students].sort((a,b)=>b.totalUnresolvedDoubts-a.totalUnresolvedDoubts):s.students}))})).filter(x=>x.subjects.length),[all,classroom,subject,sort]);
 return <main className="school-page sa-page"><style>{css}</style><div className="school-stack sa-stack">
  ...
  <section className="school-section"><div className="school-section-head"><div><p className="school-eyebrow">Exam Preparation Intelligence</p><h2 className="school-section-title">Unresolved Doubt Preparation Board</h2><p className="school-section-copy">The teacher Exam Preparation risk ledger, aggregated class and section wise for the principal.</p></div><span className="school-pill">Principal Academic Ledger</span></div>
   <div className="sa-filters">
    <F label="Class & Section"><select value={classroom} onChange={e=>setClassroom(e.target.value)}><option value="ALL">All Classes & Sections</option>{classOptions.map(x=><option key={x.key} value={x.key}>{x.label}</option>)}</select></F>
    <F label="Subject"><select value={subject} onChange={e=>setSubject(e.target.value)}><option value="ALL">All Subjects</option>{subjects.map(x=><option key={x}>{x}</option>)}</select></F>
    <F label="Time Period"><select value={period} onChange={e=>setPeriod(e.target.value as Period)}><option value="ALL">All Session</option><option value="7">Last 1 Week</option><option value="14">Last 2 Weeks</option><option value="21">Last 3 Weeks</option><option value="30">Last 30 Days</option><option value="60">Last 60 Days</option><option value="CUSTOM">Custom</option></select></F>
    <F label="Student Attention"><select value={sort} onChange={e=>setSort(e.target.value as SortMode)}><option value="DEFAULT">Default</option><option value="HIGHEST_DOUBTS">Highest Number of Doubts</option></select></F>
    {period==="CUSTOM"&&<div className="sa-custom"><F label="From"><input type="date" value={from} onChange={e=>setFrom(e.target.value)}/></F><F label="To"><input type="date" value={to} onChange={e=>setTo(e.target.value)}/></F></div>}
   </div>
   {loading?<div className="school-empty">Loading exam preparation intelligence…</div>:visible.length===0?<div className="school-empty">No unresolved not-discussed doubt data exists for the selected filters.</div>:visible.map(c=><Board key={c.classroomKey} c={c}/>)}
  </section>
 </div></main>
}
...
.sa-filters{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px;margin-top:18px;width:100%;max-width:100%;min-width:0}
...
.sa-filter select,.sa-filter input{width:100%;box-sizing:border-box;border:1px solid #E2E8F0;border-radius:11px;background:#FFF;padding:10px 11px;color:#0F172A;font-size:12px;font-weight:750}
...
@media(max-width:1024px){
 .sa-filters{grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}
...
@media(max-width:640px){
 .sa-filters{grid-template-columns:1fr;gap:8px;margin-top:14px}
 .sa-custom{grid-template-columns:1fr}
 .sa-filter label{margin-bottom:5px;font-size:8px}
 .sa-filter select,.sa-filter input{padding:8px 9px;border-radius:9px;font-size:10px}
