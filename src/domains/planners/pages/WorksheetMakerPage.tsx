...
export function WorksheetPreview({ record, onClose, editable=false, onSaveEdit }: { record: PlannerRecord; onClose:()=>void; editable?:boolean; onSaveEdit?:(payload:WorksheetPayload)=>void }) {
  ...
  const [schoolName,setSchoolName]=useState(payload.schoolName ?? "");
  const [chapter,setChapter]=useState(getChapter(record,payload,questions));
  const currentPayload: WorksheetPayload = { ...payload, schoolName, totalMarks:0, timeAllowed:"", chapter:chapter, questions:normalizeWorksheetQuestions(questions) };
...
export default function WorksheetMakerPage() {
  ...
  useEffect(()=>{
    ...
    const timer=window.setTimeout(async()=>{
      ...
      try{const rows=await getPlannerRecommendations({plannerType:WORKSHEET_PLANNER_TYPE as PlannerType,className,chapterName:chapter.trim()});if(requestId===recommendationRequest.current)setRecommendations(rows);}
  ...
  async function submit(){if(!valid())return;setSaving(true);try{await savePlanner({plannerType:WORKSHEET_PLANNER_TYPE as PlannerType,title:title.trim()||"Worksheet",templateKey:"a4-paper",className,sectionName,subjectName,startDate:date,endDate:date,payload:makePayload(),chapterName:chapter.trim(),submit:true});setQuestions([]);setChapter("");await load()}catch(e:any){setError(e?.message??"Unable to submit worksheet.")}finally{setSaving(false)}}
  async function saveEdited(payload:WorksheetPayload){if(!editing)return;setSaving(true);try{
    const isRecommendation=editing.id.startsWith("recommendation:");
    const normalizedChapter=(payload.chapter??"").trim();
    const nextPayload:WorksheetPayload={...payload,schoolName:payload.schoolName??"",totalMarks:0,timeAllowed:"",chapter:normalizedChapter||"Chapter 1",questions:normalizeWorksheetQuestions(payload.questions??[])};
    const input:any={plannerType:WORKSHEET_PLANNER_TYPE as PlannerType,title:editing.title,templateKey:editing.templateKey,className:editing.className,sectionName:editing.sectionName,subjectName:editing.subjectName,startDate:editing.startDate,endDate:editing.endDate,payload:nextPayload,chapterName:normalizedChapter||"Chapter 1",submit:true};
    if(!isRecommendation) input.id=editing.id;
    await savePlanner(input);setEditing(null);await load();
  }catch(e:any){setError(e?.message??"Unable to save worksheet edits.")}finally{setSaving(false)}}
